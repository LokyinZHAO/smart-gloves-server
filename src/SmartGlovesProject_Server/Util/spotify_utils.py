...
from SmartGlovesProject_Server.Data_Prepare.credential import client_ID, client_SECRET
...
def get_all_songs_from_df(df: pd.DataFrame, dest_dir: str):
    """
    Function that downloads all the songs mentioned in df in the directory mp3_preview
    Input:
    ...
    for row in tqdm(range(df.shape[0])):
        try:
            ...
            log.write(f"miss song:{dest_dir + song_id}")
            continue
    log.close()
