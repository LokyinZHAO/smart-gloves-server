...
def upload_to_bucket(file_dir, client: ObsClient):
    obj_name = file_dir.split('/')[-1].replace('.mp3', '.dump')
...
def music_info_gen_batch(src_dir: str):
    ...
    process_time_consume = 0
    download_time_consume = 0
    upload_time_consume = 0
    item_cnt = 0
    for i in files:
        start_time = time.time()
        download_ok = download_from_bucket(i, obs_client)
        end_time = time.time()
        download_time_consume += end_time - start_time
        if download_ok is True:
            start_time = time.time()
            music = process(i.replace('.mp3', '.wav'), pred)
            end_time = time.time()
            process_time_consume += end_time - start_time
            ...
            end_time = time.time()
            upload_time_consume += end_time - start_time
            if not upload_ok:
            ...
            continue
    log_file.close()
    return process_time_consume, download_time_consume, upload_time_consume, item_cnt
...
if __name__ == '__main__':
    ...
    total_upload_time = 0
    for res in results:
        total_item += res.get()[3]
        total_download_time += res.get()[1]
        total_upload_time += res.get()[2]
        total_process_time += res.get()[0]
    ...
    batch_log_file.write(
        f"Total process time:  {int(total_process_time // 3600)}H {int((total_process_time % 3600) // 60)}M {int(total_process_time % 60)}S\n")
    batch_log_file.write("Average process time per item: %.2fs\n" % (total_process_time / total_item))
    batch_log_file.write(
        f"Total download time:  {int(total_download_time // 3600)}H {int((total_download_time % 3600) // 60)}M {int(total_download_time % 60)}S\n")
    batch_log_file.write("Average download time per item: %.2fs\n" % (total_download_time / total_item))
    batch_log_file.write(
        f"Total upload time:  {int(total_upload_time // 3600)}H {int((total_upload_time % 3600) // 60)}M {int(total_upload_time % 60)}S\n")
    batch_log_file.write("Average upload time per item: %.2fs\n" % (total_upload_time / total_item))
